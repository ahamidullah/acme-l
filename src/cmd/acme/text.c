...
textsetselect(Text *t, uint q0, uint q1)
{
	...
	if (t->fr.l0 != t->fr.l1) {
		if (t->fr.p0 >= t->fr.l1 || t->fr.p1 <= t->fr.l0)
			frdrawsel(&t->fr, frptofchar(&t->fr, t->fr.l0), t->fr.l0, t->fr.l1, 0);
		else {
			if (t->fr.p1 < t->fr.l1 && t->fr.p1 > t->fr.l0)
				frdrawsel(&t->fr, frptofchar(&t->fr, t->fr.p1), t->fr.p1, t->fr.l1, 0);
			if (t->fr.p0 > t->fr.l0 && t->fr.p0 < t->fr.l1)
				frdrawsel(&t->fr, frptofchar(&t->fr, t->fr.l0), t->fr.l0, t->fr.p0, 0);
		}
		t->fr.l0 = t->fr.l1 = 0;
	}
