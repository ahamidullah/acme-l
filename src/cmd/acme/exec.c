...
void	lisp(Text*, Text*, Text*, int, int, Rune*, int);
...
void
lisp(Text *et, Text *_0, Text *argt, int _1, int _2, Rune *arg, int narg)
{
	Rune *a, *r;
	Window *w;
	int na, len, lisp;
	...
	if(r!=nil && len>0)
		...
		a = findbl(arg, narg, &na);
		if(a != arg)
			lisp = lispval(arg, narg-na);
	}
	if(lisp == IGlobal)
		allwindows(fixlisp, nil);
	else if(w != nil && lisp >= 0)
		w->lisp = lisp;
}
void
