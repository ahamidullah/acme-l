...
mksubfont(XFont *xf, char *name, int lo, int hi, int size, int antialias)
{
	...
	/* Manipulate the incoming size parameter by repeating the calculation
done in the main fontsrv.
 	   fontsrv believes this to be the pixel height, so set the font at that
pixel height as well
 	*/
	...
	for(i=lo; i<=hi; i++, fc++) {
		...
		e = FT_Load_Char(face, i, FT_LOAD_RENDER|(antialias ?
aa_flags:FT_LOAD_TARGET_MONO));
		if(e){
